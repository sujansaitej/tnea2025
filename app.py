...
from college_predictor import list_of_colleges, category          # keeps your other helpers
...
GOOGLE_CREDENTIALS_PATH = os.path.join(os.path.dirname(__file__),
                                       "credentials.json")

def choose_database_uri() -> str:
    """Try MySQL first; if it fails, fall back to local SQLite."""
    mysql_uri = (
    ...
    try:
        ...
        logger.warning(f"MySQL unavailable – falling back to SQLite: {e}")
...
app.config["SQLALCHEMY_ENGINE_OPTIONS"] = {
    "pool_pre_ping": True,       # auto-detect stale MySQL connections
    "pool_recycle": 1800,
}
...
with app.app_context():
    db.create_all()              # works for either MySQL or SQLite
...
def _append_to_local_csv(row):
    ...
    with open(BACKUP_CSV, "a", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(("Name", "Mobile Number", "Course", "Community", "Cutoff"))
        writer.writerow(row)
...
def insert_data(name, mobile_number, course, community, cutoff):
    try:
        duplicate = Student.query.filter_by(
            name=name, mobile_number=mobile_number, course=course,
            community=community, cutoff=cutoff
        ).first()
        if not duplicate:
        ...
        logger.error(f"DB error – using file backup: {err}")
        db.session.rollback()
...
def write_to_google_sheets(name, mobile_number, course, community, cutoff):
    """Try Sheets; otherwise drop to CSV."""
    try:
        ...
        if not ws.row_values(1):
            ws.update("A1:E1", [["Name","Mobile Number","Course","Community","Cutoff"]])
...
def results():
    try:
        ...
        # top set
        top  = data[
            data["Normalized_Branch"].isin(course_cat) & (data[comm] > 0)
        ].sort_values(by=comm, ascending=False).drop(columns=["Normalized_Branch"])
...
def filter_colleges():
    try:
        ...
        filt = data[
            ...
            (data[comm] > 0) & (data[comm] <= marks + 5)
        ].sort_values(by=comm, ascending=False).drop(columns=["Normalized_Branch"])

        top  = data[
            data["Normalized_Branch"].isin(course_set) & (data[comm] > 0)
        ].sort_values(by=comm, ascending=False).drop(columns=["Normalized_Branch"])
